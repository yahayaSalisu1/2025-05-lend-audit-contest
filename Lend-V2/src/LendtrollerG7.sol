 ...
 * @author Compound
 */
contract LendtrollerG7 is LendtrollerV5Storage, LendtrollerInterface, LendtrollerErrorReporter, ExponentialNoError {
    ...
    event LendSpeedUpdated(LToken indexed lToken, uint256 newSpeed);
     ...
     */
    function setLendSpeedInternal(LToken lToken, uint256 lendSpeed) internal {
        uint256 currentLendSpeed = lendSpeeds[address(lToken)];
        if (currentLendSpeed != 0) {
            ...
            if (lendSupplyState[address(lToken)].index == 0 && lendSupplyState[address(lToken)].block == 0) {
                lendSupplyState[address(lToken)] = LendMarketState({
                    index: lendInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }

            if (lendBorrowState[address(lToken)].index == 0 && lendBorrowState[address(lToken)].block == 0) {
                lendBorrowState[address(lToken)] = LendMarketState({
                    index: lendInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentLendSpeed != lendSpeed) {
            lendSpeeds[address(lToken)] = lendSpeed;
            emit LendSpeedUpdated(lToken, lendSpeed);
        }
    }
    ...
    function updateLendSupplyIndex(address lToken) internal {
        ...
        uint256 supplySpeed = lendSpeeds[lToken];
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            lendSupplyState[lToken] = LendMarketState({
    ...
    function updateLendBorrowIndex(address lToken, Exp memory marketBorrowIndex) internal {
        ...
        uint256 borrowSpeed = lendSpeeds[lToken];
        uint256 blockNumber = getBlockNumber();
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            lendBorrowState[lToken] = LendMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
    ...
    function distributeSupplierLend(address lToken, address supplier) internal {
        ...
        Double memory supplierIndex = Double({mantissa: lendSupplierIndex[lToken][supplier]});
        lendSupplierIndex[lToken][supplier] = supplyIndex.mantissa;
        ...
        emit DistributedSupplierLend(LToken(lToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerLend(address lToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        Double memory borrowerIndex = Double({mantissa: lendBorrowerIndex[lToken][borrower]});
        lendBorrowerIndex[lToken][borrower] = borrowIndex.mantissa;
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            emit DistributedBorrowerLend(LToken(lToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     * @param lendSpeed New LEND speed for market
     */
    function _setLendSpeed(LToken lToken, uint256 lendSpeed) public {
        require(adminOrInitializing(), "only admin can set lend speed");
        setLendSpeedInternal(lToken, lendSpeed);
    }
     ...
     */
    function getLendAddress() public pure returns (address) {
