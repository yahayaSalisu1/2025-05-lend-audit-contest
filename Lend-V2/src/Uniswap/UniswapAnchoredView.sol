...
contract UniswapAnchoredView is AggregatorValidatorInterface, UniswapConfig, Ownable {
     ...
     */
    constructor(uint256 anchorToleranceMantissa_, uint32 anchorPeriod_, TokenConfig[] memory configs)
        ...
        Ownable()
    {
        ...
        upperBoundAnchorRatio = anchorToleranceMantissa_ > type(uint256).max - ETH_BASE_UNIT
            ? type(uint256).max
    ...
    /**
     * @notice Get the underlying price of a lToken, in the format expected by the Comptroller.
     * @dev Implements the PriceOracle interface for Lend
     * @param lToken The lToken address for price retrieval
     * @return Price denominated in USD for the given lToken address, in the format expected by the Comptroller.
     */
    ...
    function fetchEthPrice() internal view returns (uint256) {
        return fetchAnchorPrice(getTokenConfigBySymbolHash(ETH_HASH), ETH_BASE_UNIT);
    }
